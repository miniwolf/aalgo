...
This results in $2N$ calls to decrease key. $N$ calls for the $N$ nodes connected from the source. And one call for each of the $N$ nodes, because the total weight from the source to the sink is $N*2 +1 -i$ at that point, where $i$ increases by one with each node visited.

\section*{Experiments - Dijkstra}

\subsection*{Hypothesis}
We expect graphs using a lot of \texttt{DECREASE-KEY} operations to be generally faster, when using Fibonacci Heaps. This is because the cost of \texttt{DECREASE-KEY} is significantly lower in the Fibonacci Heap implementation than in the Binary Heap implementation which is shown in previous tests.

In our tests we intend to show that the implementation of \texttt{FIBONACCI\--HEAP\--DECREASE\--KEY} will yield a lower increase in running time for increasing sizes of graphs.
We first run \texttt{DIJKSTRA} using the Fibonaci Heap implementation on increasing sizes of graphs, then we do the same for the Binary Heap implementation. The graphs we use are the three types of graphs explained above.

What we expect to see is that the factor of increase for the running time will growe significantly lower for the Fibonacci Heap implementation than the Binary Heap implementation due to the amortized \texttt{DECREASE-KEY} function.
\subsection*{Graphs}
