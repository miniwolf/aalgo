...
removeL (L, R, L', R') =
  if length L == 0 then (head R, [])
  else if length L > 0 then (head L, makedq (tail L, R, tail (tail L'), tail (tail R')))
removeR (L, R, L', R') =
  if length R == 0 then (head L, [])
  else if length R > 0 then (head R, makedq (L, tail R, tail (tail L'), tail (tail R')))
rot1 (n, L, R) = 
